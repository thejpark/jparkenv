...
(defun create-tags (dir-name)
  ...
  (shell-command
   (format "rm -f ./TAGS; find . -type f  \\( -iname \"\*.[chS]\" -o -iname \"\*.[ch]pp\" -o -iname \"\*.py\"  -o -iname \"\*.java\" \\) | xargs realpath | xargs etags -a -o %s/TAGS"
...
(defun my-evil-mode-hook()
  ...
  (define-key evil-normal-state-map (kbd ",h") 'mark-defun)
  ;; (define-key evil-visual-state-map (kbd ",h") 'mark-defun)
...
(defun jpark-log-edit-mode-hook ()
  (if (eq evil-mode 't) (evil-emacs-state))
)

(add-hook 'log-edit-mode-hook 'jpark-log-edit-mode-hook)
...
(global-set-key (kbd "<f2>") 'find-file-in-project)
...
;; (global-set-key (kbd "<f7>") 'keyboard-escape-quit)
(global-set-key (kbd "<f7>") 'avy-goto-char)
