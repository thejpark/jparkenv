...
(defcustom gnuplot-program "gnuplot"
  ...
  :group 'gnuplot
  :type 'string)
(defcustom gnuplot-program-arg nil
  "*The arg of the gnuplot program."
  :group 'gnuplot
  :type 'string)
...
(defvar gnuplot-send-file-history nil)
...
This sets `gnuplot-recently-sent' to 'file."
  ...
  (let ((filename (file-name-nondirectory (buffer-file-name)))
        (string))
    (setq filename (read-shell-command "file : " filename 'gnuplot-send-file-history))
    (setq string (concat "load '" (expand-file-name filename) "'\n"))
    (message "%S" string)
    (gnuplot-make-gnuplot-buffer)	; make sure a gnuplot buffer exists
...
(defun gnuplot-make-gnuplot-buffer ()
  ...
  (or (and gnuplot-process (get-process gnuplot-process)
	...
	(setq gnuplot-buffer 
	      (if gnuplot-program-arg
		  (make-comint gnuplot-process-name gnuplot-program nil gnuplot-program-arg)
